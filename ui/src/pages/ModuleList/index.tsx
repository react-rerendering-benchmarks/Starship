...
import { deleteModule, deployModule, listModule, undeployModule } from '@/services/ant-design-pro/api';
...
const ArrayRender = (data: {
  ...
  return <Form layout={(layout as any)}>
      {data.map(item => {
      if (Array.isArray(item)) {
        return ArrayRender(item, 'inline');
      }
      return <Form.Item label={item.label} key={item.value}>
            {item.value}
          </Form.Item>;
    })}
    </Form>;
};
const CodeList: React.FC = memo(() => {
  ...
  const columns: ColumnsType<CodeListItemType> = [{
    ...
    render: (_: any, columnData: any) => <>
        ...
        marginLeft: 10
          ...
          <Popconfirm placement="bottom" title={'Are you sure to delete ?'} onConfirm={async () => {
  ...
  return <PageContainer>
        ...
        expandedRowRender: columnData => {
          return <Table style={{
            ...
            render: (da: string) => {
              ...
              return <Form>
                            ...
                            <Form.Item label="perfBuffers">
                              {ArrayRender(res.kprobes?.map((i: any) => [{
                    label: 'target',
                    value: i.target
                  }, {
                    label: 'entry',
            ...
            render: (da: string) => {
              ...
              const res: any = JSON.parse(da || '[]');
              return ArrayRender(res?.map((i: any) => ({
                label: i.name,
                value: i.type
              })));
            }
      ...
      }} rowKey={(item: any) => item.ID} dataSource={data} />
      </Card>
    </PageContainer>;
});
